...
#==============================================================================#
# Architecture (rewritten to match this project's own Cable Club plugin):
# BOTH clients construct their own full, real, locally-animated `Battle`
# instance — each sees themselves as the "player" side and the other as the
# "opponent" side. A shared RNG seed (generated by whoever issued the
# challenge, sent to the other side) is fed into both instances' random
# rolls, so two independent simulations given the same inputs produce
# identical results. Each side drives their OWN Pokemon through the normal
# interactive Fight/Bag/Pokemon/Run menu — no networking needed for that at
# all. The ONLY thing exchanged over the network, once per round, is each
# side's already-decided move/switch choice for their own Pokemon, which
# gets written directly into the OTHER side's @battle.choices for the
# opposing battler index — exactly what Cable Club's own AI hook does.
#==============================================================================#
...
module Online
  ...
  #=============================================================================
  def self.request_battle_challenge(target_player)
    ...
    puts "[Battle] Challenge sent to #{target_player.name}, battle_id=#{battle_id}"
    battle_id
  end
  ...
  def self.wait_for_battle_choice(battle_id, round)
    ...
    loop do
      ...
      WSClient.poll.each do |msg|
        ...
        case msg["action"]
        when "battle_choice"
          return msg if msg["round"].to_i == round
        when "battle_cancel"
          return nil
        end
      end
...
class Game_Player
  def handle_incoming_battle_challenge(msg)
    ...
    if pbConfirmMessage("#{other_name} wants to battle! Accept?")
      WSClient.send_json({
      ...
      puts "[Battle] battle_accepted sent for #{battle_id}"
      ...
      pbMessage("Waiting for #{other_name}'s party...")
